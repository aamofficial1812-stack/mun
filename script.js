...
(function () {
  ...
  /* ISO 3166-1 alpha-2 codes, in the exact same order as UNHRC_PORTFOLIOS
     above, used only to look up each country's flag. The formal diplomatic
     names above are never altered — this is a separate, silent mapping. */
  var UNHRC_FLAG_CODES = [
    "dz", "ao", "au", "bo", "bw", "br", "ca", "cl", "cn", "cr",
    "cu", "kp", "dk", "fi", "de", "il", "it", "jp", "la", "mu",
    "mx", "mz", "np", "nz", "ni", "no", "qa", "sg", "za", "kr",
    "es", "lk", "se", "ch", "tz", "ae", "gb", "us", "uy", "ve",
    "vn", "sa"
  ];

  /* countryFlags: formal country name -> ISO code, built by zipping the
     two arrays above so the mapping can never drift out of sync. */
  var countryFlags = {};
  UNHRC_PORTFOLIOS.forEach(function (name, i) {
    countryFlags[name] = UNHRC_FLAG_CODES[i];
  });
  ...
  function renderUnhrcPortfolio(filterText) {
    ...
    if (matches.length === 0) {
      ...
      matches.forEach(function (name) {
        ...
        var code = countryFlags[name];
        if (code) {
          var flagEl = document.createElement('img');
          flagEl.className = 'pc-flag';
          flagEl.src = 'https://flagcdn.com/w80/' + code + '.png';
          flagEl.srcset = 'https://flagcdn.com/w80/' + code + '.png 1x, https://flagcdn.com/w160/' + code + '.png 2x';
          flagEl.alt = '';
          flagEl.loading = 'lazy';
          flagEl.width = 28;
          flagEl.height = 21;
